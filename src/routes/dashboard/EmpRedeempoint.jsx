...
const EmpRedeempoint = () => {
    const location = useLocation();
    const { LManagerEmployeeId } = location.state;
    const [employee, setemployee] = useState([{}]);
    const fetchemployeeData = async () => {
        try {
            const response = await API.get(`/getredeemrequest/${LManagerEmployeeId}`, {
                headers: {
                    "Content-Type": "application/json",
                }, 
            });
            setemployee(response.data.request);
    ...
    return (
        <div className="flex min-h-screen flex-col gap-y-4 p-6">
            <div>
                <h1 className="mb-4 text-2xl font-bold text-gray-800 dark:text-white">Employee Redeem Point Request</h1>
